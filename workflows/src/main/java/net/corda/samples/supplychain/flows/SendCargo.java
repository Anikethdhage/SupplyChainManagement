...
//    private PurchaseOrderData cargo;
    ...
    //public constructor
    public SendCargo(String pickupFrom, String shipTo, int purchaseOrderNumber){
...
//        this.cargo = cargo;
    ...
    public String call() throws FlowException {
        ...
        StateAndRef<ShippingRequestState> shippingRequestStateStateAndRef = getServiceHub().getVaultService().queryBy(ShippingRequestState.class).getStates()
                .stream().filter(it -> it.getState().getData().getCargo().getPurchaseOrderNumber() == purchaseOrderNumber).findFirst().get();

        PurchaseOrderData purchaseOrderData = shippingRequestStateStateAndRef.getState().getData().getCargo();

        //generating State for transfer
        CargoState output = new CargoState(new AnonymousParty(myKey),targetAcctAnonymousParty,purchaseOrderData,getOurIdentity());
        ...
        TransactionBuilder txbuilder = new TransactionBuilder(notary)
                .addOutputState(output, CargoStateContract.ID)
        ...
        return "send " + purchaseOrderData.toString()+ " to " + targetAccount.getHost().getName().getOrganisation() + "'s "+ targetAccount.getName() + " team";
